...
void	scene_win(struct nk_context *ctx, t_scene *sc)
{
	...
	if (!float3_equal(prev_pos, sc->cam.pos) || !float3_equal(prev_rot, sc->cam.rot)
		|| prev_ambient != sc->ambient || prev_max_ref != sc->max_reflections)
		redraw_scene(sc);
}
...
void	obj_win(struct nk_context *ctx, t_obj *obj, t_scene *sc)
{
	if (nk_begin(ctx, "Object", nk_rect(10, 10, 420, 340),
		...
		obj->type = nk_combo(ctx, sc->obj_types, OBJ_TYPES_COUNT, obj->type, 25, nk_vec2(120,200));
		...
		nk_property_int(ctx, "", 0, &obj->radius, 100.0f/*max*/, 1, 1);
		...
		nk_label(ctx, "Distanse:", NK_TEXT_LEFT);
		if (nk_input_is_mouse_hovering_rect(&ctx->input, bounds))
			nk_tooltip(ctx, "Distanse from cam to object, that ray cast returned");
		nk_labelf(ctx, NK_TEXT_LEFT, "%f" , obj->t);
